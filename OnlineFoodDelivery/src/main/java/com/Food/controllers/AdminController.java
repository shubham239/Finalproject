...
public class AdminController {
	...
	//Update 	product
	@GetMapping(path = {"/updateproduct","/updateproduct/{id}"})
	public String product(Model model,@PathVariable("id") Optional<Integer> id) {
		if(id.isPresent()) {
			Product pro=prodsrv.findProductById(id.get());
					//.findByCatId(id.get());
			model.addAttribute("prodid",pro. getProdid());
			...
			//model.addAttribute("prodid", prodsrv.generateCatId());

		}
		model.addAttribute("pro", prodsrv.allProducts());		
		return "product";
	}	
